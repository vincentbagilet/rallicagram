...
### Co-occurrences in two lexicons

If one wants to compute co-occurrences in two lexicons, one can use the function `gallicagram_cooccur_lexicon`. It simply loops the function `gallicagram_cooccur` over each  word of each lexicon and sums the results. It takes as inputs two vectors of keywords:

```{r ex_cooccur_lexicon}
...
```{r kable_cooccur_lexicon, echo=FALSE}
ex_cooccur_lexicon |>
  utils::head() |>
  knitr::kable()
```

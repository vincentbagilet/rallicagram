...
We can use `geom_smooth` to plot smoother lines. The parameter `span` controls the smoothing, larger values being associated with less smoothing. Here is the same graph but smoothed:

```{r smoothed_ts, message=FALSE}
ex_data |> 
  ggplot(aes(x = date, y = prop_occur)) + 

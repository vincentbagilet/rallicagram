...
ex_associated <- gallicagram_associated(
  ...
  from = 1960,
  to = 1970,
  nb_associated_words = 10
)
```

The most often associated terms are:
...
We can see that there are stopwords included in this list. Since they may not be informative, we may want to remove them. We can do so by supplying a list of stopwords to the `stopwords` parameter of the function. For instance, we can use the list in the `lsa` package, `lsa::stopwords_fr`:
...
Note that it reduces the number of associated words returned and that it does not correspond to `nb_associated_words` anymore.
